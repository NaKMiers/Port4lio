...
describe('career content coverage', () => {
  it('covers all sixteen types in every locale', () => {
    for (const locale of LOCALES)
      for (const type of MBTI_TYPES) {
        ...
        expect(
          careers.workStyle.length,
          `${locale} ${type} workStyle`
        ).toBeGreaterThan(80)
        expect(
          careers.thrivesIn.length,
          `${locale} ${type} thrivesIn`
        ).toBeGreaterThan(40)
        expect(
          careers.drainedBy.length,
          `${locale} ${type} drainedBy`
        ).toBeGreaterThan(40)
        expect(
  ...
  it('never repeats a role list between two types', () => {
    ...
    for (const locale of LOCALES) {
      ...
      for (const type of MBTI_TYPES) {
        ...
        expect(
          seen.has(key),
          `${locale}: ${type} has the same roles as ${seen.get(key)}`
        ).toBe(false)
        seen.set(key, type)
      }
    }
    ...
    for (const locale of LOCALES)
      for (const a of MBTI_TYPES)
        for (const b of MBTI_TYPES) {
          ...
          expect(
            shared,
            `${locale}: ${a} and ${b} share ${shared} roles`
  ...
  it('never repeats a workStyle sentence between two types', () => {
    for (const locale of LOCALES) {
      ...
      for (const type of MBTI_TYPES) {
        ...
        expect(
          seen.has(style),
          `${locale} ${type} duplicates another workStyle`
        ).toBe(false)
        seen.add(style)
      }
    }
  ...
  it('gives each locale its own list rather than a translation of the other', () => {
    ...
    for (const type of MBTI_TYPES)
      expect(getCareerContent('vi', type).roles).not.toEqual(
        getCareerContent('en', type).roles
      )
  })
  ...
  it('describes what drains a type differently from what suits it', () => {
    ...
    for (const locale of LOCALES)
      for (const type of MBTI_TYPES) {
        const careers = getCareerContent(locale, type)
        expect(careers.thrivesIn, `${locale} ${type}`).not.toBe(
          careers.drainedBy
        )
      }
  })

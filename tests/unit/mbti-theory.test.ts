...
describe('letters', () => {
  it('decomposes a type into its four letters in order', () => {
    expect(lettersOf('vi', 'ENTJ').map(l => l.letter)).toEqual([
      'E',
      'N',
      'T',
      'J',
  ...
  it('covers every type without a missing definition', () => {
    for (const locale of LOCALES)
      for (const type of MBTI_TYPES) {
        ...
        expect(
          entries.every(e => e.summary.length > 0),
          `${locale} ${type}`
        ).toBe(true)
      }
  })
  ...
  it('describes all eight functions in every locale', () => {
    for (const locale of LOCALES)
      for (const fn of [
        ...
        expect(
          FUNCTION_COPY[locale][fn].name.length,
          `${locale} ${fn}`
        ).toBeGreaterThan(3)
        expect(
          FUNCTION_COPY[locale][fn].summary.length,
          `${locale} ${fn}`
        ).toBeGreaterThan(30)
      }
  })
...
describe('per-type FAQ', () => {
    ...
    for (const locale of LOCALES)
      for (const type of MBTI_TYPES) {
        ...
        for (const { q, a } of entries) {
          ...
          expect(a, `${locale} ${type} unfilled placeholder`).not.toMatch(
            /\{[a-z]+\}/
          )
        }
      }
  ...
  it('uses the right indefinite article in English', () => {
    ...
    for (const type of MBTI_TYPES) {
      const questions = typeFaqEntries('en', type)
        .map(e => e.q)
        .join(' ')

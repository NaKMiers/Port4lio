  ...
  it('a repeat view updates ONE document rather than adding a second', async () => {
    ...
    for (let i = 0; i < 5; i += 1)
      await recordPostEvent({
        kind: 'view',
        slug: 'a-post',
        subject: 'a-post:session-1',
      })

    expect(await PostEventModel.countDocuments({})).toBe(1)
    const row = await PostEventModel.findById(
      'blog:view:a-post:session-1'
    ).lean()
    expect(row?.count, 'count should record the re-fires as a diagnostic').toBe(
      5
    )
  })
...
describe('retention', () => {
  it('is 180 days, deliberately NOT the 21 TestEvent uses', async () => {
    ...
    const days =
      (postEventExpiryFrom(now).getTime() - now.getTime()) / 86_400_000
  ...
  it('builds the TTL index, because a failed build is silent', async () => {
    const indexes = (await PostEventModel.collection.indexes()) as Record<
      string,
      unknown
    >[]
  ...
  it('indexes the aggregation the board actually runs', async () => {
    const keys = (
      (await PostEventModel.collection.indexes()) as Record<string, unknown>[]

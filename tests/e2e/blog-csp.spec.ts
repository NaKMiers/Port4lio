...
test('CSP is on /blog itself, not only on its children', async ({
  request,
}) => {
  ...
  expect(child.headers()['content-security-policy']).toContain(
    "frame-ancestors 'none'"
  )
})

test('the CSP names the directives that do not fall back to default-src', async ({
  request,
}) => {
  const csp =
    (await request.get('/blog')).headers()['content-security-policy'] ?? ''
...
test('the page HYDRATES under that CSP', async ({ page }) => {
  ...
  await page.waitForFunction(
    () => document.querySelector('body')?.innerHTML.length ?? 0 > 0
  )

  expect(
    violations,
    `CSP blocked something in a real browser:\n${violations.join('\n')}`
  ).toEqual([])
})

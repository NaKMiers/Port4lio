 ...
 *                                                 ──▶ { cvs, publishedId }
 *   POST  requireOwner ──▶ readJsonBody (256 KB) ──▶ cv-service.createCv({ label, fromId })
...
export async function POST(request: NextRequest) {
  ...
  const parsed = await readJsonBody<{ label?: unknown; fromId?: unknown }>(
    request,
    { maxBytes: MAX_CV_JSON_BYTES }
  )
  if (!parsed.ok) return jsonError(parsed.error, parsed.status)
  const body = parsed.body ?? {}

  try {
    const result = await createCv({ label: body.label, fromId: body.fromId })

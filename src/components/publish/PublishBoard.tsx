...
export default function PublishBoard() {
  const [manifest, setManifest] = useState<PublishManifestWithDrift | null>(
    null
  )
  ...
  const load = async (blocking: boolean) => {
    try {
      ...
      if (!res.ok)
        throw new Error(data?.error || 'Failed to load publish manifest')
      setManifest(data as PublishManifestWithDrift)
    } catch (e) {
      setError(
        e instanceof Error ? e.message : 'Failed to load publish manifest'
      )
    } finally {
  ...
  const targets = manifest
    ? manifest.targetOrder.map(id => manifest.targets[id])
    : []
          ...
          <div className="max-w-3xl space-y-3">
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <span className="rounded-full bg-pp-text px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white">
                {behind === 0
                  ? 'Everything in sync'
                  : `${behind} of ${targets.length} out of date`}
              </span>
            </div>
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-pp-muted sm:text-base">
                GitHub is updated automatically by a scheduled workflow.
                LinkedIn, Upwork and Fiverr publish no API for editing a
                profile, so those are copy-and-paste - press Mark as pasted once
                you have, and this page will tell you when they drift again.
              </p>
            </div>
          </div>

  ...
  return (
    <div className="space-y-3">
      <p className={helpTextCls}>
        Applied automatically by the <strong>publish-profile</strong> workflow.
        Nothing to paste - this is a preview of what it will write.
      ...
      {artifact.kind === 'fields-object' ? (
        <dl className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {(['name', 'bio', 'blog', 'location', 'company'] as const).map(
            key => (
              <div
                key={key}
                className="rounded-[1rem] border border-pp-line bg-white/60 px-3 py-2"
              >
                <dt className={labelCls}>{key}</dt>
                <dd className="break-words text-sm text-pp-text">
                  {artifact[key] || '-'}
                </dd>
              </div>
            )
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className={`${helpTextCls} mt-0.5`}>
            {isManual ? 'Copy and paste - no write API exists' : 'Automated'} ·
            v{target.version.slice(0, 8)}
          </p>
        </div>

...
export default function ProfileTabPreview({
  ...
  const statNote = limitNote(
    hero.stats.length,
    HERO_STAT_LIMIT,
    'the hero prints the first 3'
  )
  ...
  return (
    <div className="space-y-5">
        ...
        >
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-end gap-2.5">
            ...
            <div className="min-w-0 pb-0.5">
              ...
              >
                {titleLine ||
                  (hero.username ? `@${hero.username}` : 'No job titles yet')}
              </div>
            </div>
          </div>
        </div>
      ...
      <PreviewBlock title="Headline">
        ...
        {hero.subheadline ? (
          <p className="text-[13px] leading-relaxed text-pp-muted">
            {hero.subheadline}
          </p>
        ) : null}
      ...
      <PreviewBlock
        title="Stat tiles"
        count={hero.stats.length}
      >
        {hero.stats.length === 0 ? (
          <EmptyHint>
            No stats yet - the hero metric row and the “At a glance” band stay
            hidden.
          </EmptyHint>
        ) : (
          <>
            <div className="grid grid-cols-3 gap-2">
              {hero.stats.slice(0, HERO_STAT_LIMIT).map(stat => (
                <div
                  key={stat.label}
                  className="bg-white/78 rounded-[0.9rem] border border-pp-line px-2 py-2"
                >
            ...
            {statNote ? (
              <CutoffNote>
                {statNote}. All of them appear in the trust band.
              </CutoffNote>
            ) : null}
          </>
        )}
      ...
      <PreviewBlock
        ...
        count={socialProof.profiles.length}
      >
        {socialProof.profiles.length === 0 ? (
          <EmptyHint>
            No usable links - the social strip is not rendered at all.
          </EmptyHint>
        ) : (
          <>
        ...
        {socialDropped > 0 ? (
          <CutoffNote>
            {socialDropped} link{socialDropped > 1 ? 's' : ''} hidden from the
            live site - each needs an http(s), mailto, tel or root-relative URL.
          </CutoffNote>
        ) : null}
      </PreviewBlock>
      ...
      <PreviewBlock
        title="Story"
        count={about.paragraphs.length}
      >
        {about.paragraphs.length === 0 ? (
          <EmptyHint>
            About Me is empty - the story panel and the pull quote stay hidden.
          </EmptyHint>
        ) : (
          <PreviewCard>
            <p className="text-[13px] leading-relaxed text-pp-text">
              {about.paragraphs[0]}
            </p>
      ...
      {founderProof.quote ? (
        <PreviewBlock title="Pull quote">
          <PreviewCard className="border-l-2 border-l-pp-violet">
            ...
            <p className={`mt-2 ${previewEyebrowCls}`}>
              - {founderProof.attributionName}
            </p>
          </PreviewCard>
          <CutoffNote>
            Lifted automatically from the last paragraph of About Me - edit that

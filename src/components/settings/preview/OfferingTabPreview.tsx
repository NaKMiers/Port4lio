...
export default function OfferingTabPreview({
  ...
  const heading =
    services.heading ||
    (services.items.length || services.briefBullets.length
      ? 'Capabilities'
      : '')
    ...
    <div className="space-y-5">
      ...
      >
        ...
        {!services.heading && heading ? (
          <CutoffNote>
            No service heading set, so the section prints “Capabilities”.
          </CutoffNote>
        ) : null}

        {services.briefBullets.length > 0 ? (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {services.briefBullets
              .slice(0, SERVICE_BULLET_LIMIT)
              .map((bullet, index) => (
        ...
        {bulletNote ? (
          <CutoffNote>
            {bulletNote}. All of them print again in the “Also ship” band.
          </CutoffNote>
        ) : null}
        ...
        {services.items.length === 0 ? (
          services.briefBullets.length === 0 ? (
            <EmptyHint>
              No services or bullets - the whole capabilities section is hidden.
            </EmptyHint>
          ) : (
            <CutoffNote>
              No service cards, so the bullets above render as the
              section&apos;s own cards instead.
            </CutoffNote>
          )
        ) : (
            ...
            {services.items.map((item, index) => (
              <PreviewCard key={index}>
                <div className="flex items-start gap-2.5">
                  <span className="bg-white/86 flex h-8 w-8 shrink-0 items-center justify-center rounded-[0.7rem] border border-pp-line text-pp-text">
      ...
      <PreviewBlock
        title="Featured work"
        count={featuredProjects.projects.length}
      >
        {featuredProjects.heading ? (
          ...
          <CutoffNote>
            No work heading set, so the section prints “Selected work”.
          </CutoffNote>
        )}
        {featuredProjects.projects.length === 0 ? (
          <EmptyHint>
            No renderable projects - the featured band is hidden.
          </EmptyHint>
        ) : (
          <div className="space-y-2">
      ...
      {projectCatalog.projects.length > 0 ? (
        <PreviewBlock
          title="Archive"
          count={projectCatalog.projects.length}
        >
          <CutoffNote>
            Projects {FEATURED_PROJECT_LIMIT + 1}+ render in a separate “More
            shipped stories” grid: no tech chips, one CTA, and the overview cut
            to {CATALOG_EXCERPT_CHARS} characters.
          </CutoffNote>
          <div className="space-y-2">
    ...
    <PreviewCard>
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {project.overview ? (
            <p className="mt-1 text-[11px] leading-relaxed text-pp-muted">
              {project.overview}
            </p>
          ) : (
      ...
      {(project.techStack ?? []).length > PROJECT_TECH_LIMIT ? (
        <p className="mt-1.5 text-[10px] text-pp-muted">
          {PROJECT_TECH_LIMIT} of {(project.techStack ?? []).length} tech chips
          print.
        </p>
      ) : null}
...
function CatalogProjectTile({ project }: { project: ProjectItem }) {
  ...
  const excerpt =
    body.length > CATALOG_EXCERPT_CHARS
      ? excerptText(body, CATALOG_EXCERPT_CHARS)
      : body

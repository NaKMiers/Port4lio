...
export default function CareerTabPreview({
  ...
  const experienceEntries: TimelineEntry[] = sortExperienceRecent(
    experience
  ).map((item, index) => ({
    key: `exp-${index}`,
  ...
  const educationEntries: TimelineEntry[] = sortEducationRecent(education).map(
    (item, index) => ({
      key: `edu-${index}`,
      kicker: formatPortfolioPeriod(item.start, item.end),
  ...
  const certificateEntries: TimelineEntry[] = certificates.map(
    (item, index) => ({
      key: `cert-${index}`,
  ...
  return (
    <div className="space-y-5">
      ...
      >
        {skills.length === 0 ? (
          <EmptyHint>
            No skill groups - the capability column is not rendered.
          </EmptyHint>
        ) : (
            ...
            {skills.map((group, index) => (
              <PreviewCard key={index}>
                <p className={previewEyebrowCls}>
                  {group.groupName || 'Skills'}
                </p>
                {group.items.length === 0 ? (
                  <p className="mt-1.5 text-[11px] text-pp-muted">
        ...
        {dropNote(
          ...
          skills.length,
          'a group name or at least one item'
        ) ? (
          <CutoffNote>
            {dropNote(
              profile.skills.length,
        ...
        {skillItemCount > 0 ? (
          <CutoffNote>
            Icons are not printed here - the public page renders skill items as
            text chips, so the icon you pick per item never reaches it.
          </CutoffNote>
        ) : null}
      </PreviewBlock>
      ...
      <PreviewBlock title="Career timeline">
        {!hasAnyTimeline ? (
          <EmptyHint>
            No experience, education or certificates - the whole timeline block
            is hidden.
          </EmptyHint>
        ) : (
        ...
        <PreviewCard key={entry.key}>
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              {entry.kicker ? (
                <p className={previewEyebrowCls}>{entry.kicker}</p>
              ) : null}
            ...
            <span className="bg-white/78 shrink-0 rounded-full border border-pp-line px-1.5 py-0.5 text-[9px] font-semibold tabular-nums text-pp-muted">

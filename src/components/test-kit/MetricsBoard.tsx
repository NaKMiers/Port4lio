    ...
    <div className='space-y-10'>
      ...
      */}
      <p className='text-xs leading-relaxed text-pp-muted'>
        ...
        <code>paywall-seen</code> as mutually exclusive, so its arrivals are the sum of the
        two. IQ emits <code>result-viewed</code> for every arrival and{' '}
        <code>paywall-seen</code> as the locked subset of it. Conversion is{' '}
        <code>paid ÷ (result-viewed + paywall-seen)</code> for MBTI and{' '}
        <code>paid ÷ result-viewed</code> for IQ. IQ&apos;s <code>paywall-seen</code> only
        starts counting from now, so it will read low against its older{' '}
        <code>result-viewed</code> total for a while.
      </p>
    </div>
  )
}
...
function ProductSection({ label, metrics }: { label: string; metrics?: ProductMetrics }) {
  if (!metrics) {
    return (
      <section className={PANEL}>
        <h2 className='font-display text-xl font-semibold text-pp-text'>{label}</h2>
        <p className='mt-3 text-pp-muted'>No data for this product yet.</p>
      </section>
    )
  }

  const steps = FUNNEL_STEPS.map(step => ({ ...step, value: metrics.funnel[step.event] ?? 0 }))
  const empty =
    metrics.shares === 0 &&
    ...
    <section className={`${PANEL} space-y-8`}>
      ...
      {empty ? (
        ...
        <p className='text-pp-muted'>
          Nothing recorded yet. Numbers appear once someone reaches a result or shares one.
        </p>
      ) : null}

      <Group title='Funnel'>
        {steps.map(step => (
          <Row key={step.event} label={step.label} value={step.value} />
        ))}
      </Group>
      ...
      >
        <Row label='Shares' value={metrics.shares} />
...
function Group({
  ...
  return (
    <div>
      ...
      {footnote ? <p className='mt-3 text-xs text-pp-muted'>{footnote}</p> : null}
    </div>
  )
}
...
function Row({ label, value }: { label: string; value: number | string }) {
  return (
    <div className='flex items-baseline justify-between gap-6 border-b border-pp-line pb-2'>
      ...
      <dd className='font-display text-lg font-semibold tabular-nums text-pp-text'>{value}</dd>
    </div>
  )
}

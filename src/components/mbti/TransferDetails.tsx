...
export default function TransferDetails({
  ...
  const rows: {
    ...
    ...(bankName
      ? [{ key: 'bank', label: copy.bank, value: bankName, copyable: false }]
      : []),
    {
      key: 'account',
    ...
    <div>
      ...
      <dl className="mt-3 divide-y divide-pp-line rounded-panel border border-pp-line bg-white/60">
        {rows.map(row => (
          ...
          >
            <dt className="text-xs uppercase tracking-[0.12em] text-pp-muted">
              {row.label}
            </dt>
      ...
      {transferDescription && (
        <p className="mt-3 flex gap-2.5 text-xs leading-relaxed text-pp-muted">
          <span
            className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-pp-orange"
            aria-hidden
          />

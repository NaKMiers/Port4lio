...
/**
 * Email capture, then on-site payment.
 *
 * ```
 *   email form ──▶ POST /api/mbti/checkout ──▶ QR + transfer details
 *                                                      │
 ...
 *
 * On success this calls `router.refresh()` rather than pushing a route: the result page is
 * already `force-dynamic`, so refreshing re-runs the server component, which now sees
...
export default function ResultPaywall({
  ...
  const startCheckout = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        const response = await fetch('/api/mbti/checkout', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token, email, locale }),
        })
        ...
        if (!response.ok) {
          setError(response.status === 400 ? copy.invalidEmail : (data?.error ?? copy.genericError))
          setStatus('idle')
          return
        }
    ...
    [copy, email, locale, router, status, token]
  )
        ...
        <form onSubmit={startCheckout} className='mt-6'>
          ...
          <div className='mt-2 flex flex-col gap-2.5 sm:flex-row'>
            <input
              ...
              className='min-w-0 flex-1 rounded-full border border-pp-line bg-white/80 px-5 py-3 text-sm text-pp-text outline-none transition placeholder:text-pp-muted/70 focus:border-[rgba(123,109,255,0.6)] focus:ring-2 focus:ring-[rgba(123,109,255,0.25)]'
            />
            <button
              type='submit'
              disabled={status === 'creating'}
              className={cx(
                'inline-flex items-center justify-center gap-2 rounded-full bg-pp-text px-6 py-3 font-display text-xs font-semibold uppercase tracking-[0.16em] text-[var(--pp-bg)] shadow-[0_14px_28px_rgba(31,28,26,0.16)] transition',
                status === 'creating'
          ...
          <p className='mt-2.5 text-xs text-pp-muted'>{copy.emailHint}</p>
        </form>
      ) : null}

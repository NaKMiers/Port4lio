...
- **D11 (token management, owner ask 2026-09-24).** `/admin/agents` can change an unrevoked `p4_` token's scopes in place (`PATCH /api/admin/agents/tokens/<id>`, 1+ of the four scopes, the same rule as create). It applies from the token's next call; an `illustrate_post` run already drawing keeps the live-post decision it made at its start, and revoke is still what stops a run. A revoked token of either kind can be deleted for good (`DELETE ...?forever=1`, 409 while active, the same rule as the whiteboard's own route); the Activity feed keeps naming it, because each AgentAction row stores its own token name.
- **D12 (whiteboard composition, owner ask 2026-09-25).** Two write tools and a prompt, so an agent can build a whole board instead of a strip of cards. `whiteboard_compose` takes an outline (sections -> frames, cards, an optional heading shape, links by ref) and the server lays it out (`src/lib/whiteboard/compose-layout.ts`: content-height estimates, masonry inside frames, `columns` / `grid` / `timeline` / `mindmap`, arrow sides from the geometry); everything is validated before the first write (a planned id already on another board included); what can still fail after a write - a database error, or an existing link end the owner deleted mid-call - says the items landed. A clientRef plus a hash of the arguments derives the ids, so a retried partial write converges, and a key reused with a different outline after the 24 h replay window gets fresh ids instead of silently re-answering the old board. The owner allowed two things the design had ruled out: an agent may **create a board** (`newBoard`, created `includeInAi: true`), and may **move, resize or delete its own items** (`whiteboard_arrange`). D1's line still holds - arrange refuses any id that is not visible AND tagged `agent`, for the whole call, and any agent frame that holds a card without the tag (moving or deleting a frame moves or un-frames its children), so no owner-written card can change. The tag is the ownership test and the owner's to edit: removing `agent` from a card claims it. There is no hidden `createdBy` field on purpose (it would change the backup format and `visible.ts`); the /review of 2026-09-25 raised it, and it is the upgrade path if the tag proves too soft. Per-token buckets: `WHITEBOARD_COMPOSE_LIMIT` (20/h) and `WHITEBOARD_AGENT_WRITE_LIMIT` (200/10 min, now also on `whiteboard_add_item` and `whiteboard_link`, which had none). Prompt: `build-whiteboard`. Tests: `tests/unit/whiteboard-compose-layout.test.ts`, `tests/api/mcp-whiteboard-compose.test.ts`.

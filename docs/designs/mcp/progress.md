...
## Acceptance walk - done (2026-09-24)

- Setup: `next build && next start` on :3200 against a disposable `port4lio_acceptance` DB (never the `.env` database), seeded profile / posts / events / orders / whiteboard / CCA-F; headless Claude Code with `--strict-mcp-config --tools ""`, one token per scene.
- All ten asks met. 1: draft with cover + one inline image (capped at two images for cost), 404 publicly, `create_draft` + `illustrate_post` rows. 2: 200 on the first load with images, in `/blog` and RSS; a write-only token has no `publish_post` and changes nothing. 3: figures match the seed; month drops the funnel with the TTL reason. 4: one `get_me`, nothing hidden. 5: tailored CV as markdown, no `update_profile`. 6: only the typo changed, stale version refused, `/` fresh on the first load. 7: exactly the old unread post archived, 404 and gone from `/blog` and RSS. 8: visible `agent`-tagged goal linked to Career 2027; a hidden card is not found and nothing lands in a hidden frame. 9: asks for the raw count for "720", logs 42/60, reports 730. 10: order facts without personal data; without `pii` not listed, and by name refused with a feed row.
- Fixes it drove (`0b7ed5f` and the archive wording): `update_profile` expires the profile cache instead of marking it stale (ask 6 showed the typo once more); `get_me` / `get_profile(resume)` return `deriveResume`, not `null` (ask 4); `archive_post`'s "leaves /blog" was read as "stays in /blog" by the agent, reworded.
- Observed, not changed: tool results build URLs from the configured site origin, so a local run reports `https://anhkhoa.info/...` links.

...
- A board is shared **by link and nothing else**: `share` is `off` (default, and every new board starts there), `view` or `edit`. There is no invite list and no account - the URL is the credential. The owner sets it from the **Share** menu in the canvas top bar, which also renames the link.
